...
//----------基本设置页面的插件的js 向上向下的开始	-----
	jQuery('#change_up').click(function(){
		...
		var first=that.eq(0);
		var last=that.last();
		var sepcial=first.hasClass('select');
		var selectedTrLength=jQuery('#next_step_tab').find('.select').length;		
		if(that.hasClass('select')){
			if(sepcial==false){
				for(var i=0;i<cloneTrlength;i++){
					if(that.eq(i).hasClass('select')==true){
	...
	jQuery('.change_down').click(function(){
		...
		var first=that.eq(0);
		var last=that.last();
		var sepcial=last.hasClass('select');
		var selectedTrLength=jQuery('#next_step_tab').find('.select').length;		
		if(that.hasClass('select')){
			if(sepcial==false){
				for(var i=0;i<cloneTrlength;i++){
